...
//     console.log('Fetching shows with filter:', { cinema: cinemaId, ...dateFilter });
...
//     console.log(`Found ${shows.length} shows for cinema ${cinemaId}`);
...
//     const movieShows = Object.values(groupedShows);
    
//     console.log(`Returning ${movieShows.length} movie groups with shows`);
...
//   createShow
// };
// controllers/showController.js - FIXED VERSION
...
const getShowsByCinema = async (req, res) => {
  try {
    ...
    console.log('Fetching shows with filter:', filter);
    ...
    // Filter out shows with null/deleted movies and log them
    const validShows = shows.filter(show => {
      if (!show.movie) {
        console.log(`Found show with deleted movie: ${show._id}, deleting...`);
        // Delete orphaned show asynchronously
        Show.findByIdAndDelete(show._id).catch(err => 
          console.error('Error deleting orphaned show:', err)
        );
        return false;
      }
      return true;
    ...
    // Group shows by movie
    const groupedShows = validShows.reduce((acc, show) => {
      if (!show.movie || !show.movie._id) {
        return acc; // Skip if movie is still null
      }
...
const getShowById = async (req, res) => {
  try {
    ...
    if (!show.movie) {
      return res.status(404).json({
        success: false,
        message: 'Show movie not found (may have been deleted)'
      });
    }
...
// Helper function to clean up orphaned shows
const cleanupOrphanedShows = async () => {
  try {
    console.log('Starting cleanup of orphaned shows...');
    ...
    console.log(`Cleanup completed. Deleted ${deletedCount} orphaned shows.`);
    return deletedCount;
  } catch (error) {
