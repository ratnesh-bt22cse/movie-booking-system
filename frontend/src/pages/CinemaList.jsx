...
const CinemaList = () => {
  ...
  useEffect(() => {
    const handleFocus = () => {
      console.log('Window focused - refreshing cinemas');
      fetchCinemas();
    };
  ...
  const fetchCinemas = async () => {
    try {
      ...
      const response = await cinemaAPI.getAll();
      setCinemas(response.data.data.cinemas);
      console.log('Fetched cinemas:', response.data.data.cinemas);
    } catch (error) {
      console.error('Error fetching cinemas:', error);
      toast.error('Failed to load cinemas');
    } finally {
  ...
  const handleRefresh = () => {
    toast.loading('Refreshing cinemas...', { id: 'refresh' });
    fetchCinemas().finally(() => {
      toast.dismiss('refresh');
      toast.success('Cinemas refreshed!');
    });
  };
